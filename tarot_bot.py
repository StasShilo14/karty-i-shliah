...
MENU_BUTTONS = {
    "uk": {"card": "🃏 Карта дня (Таро)", "spread3": "🔮 Минуле · Теперішнє · Майбутнє",
           "love": "💞 Стосунки", "decision": "🎯 Рішення (15⭐)", "celtic": "✨ Кельтський хрест (40⭐)",
...
async def send_one_lenormand_card(message: Message, card: dict, position: str, lang: str):
    """Надсилає одну карту Ленорман текстом (фото — окремий крок пізніше, як і для Молодших Арканів)."""
    text = await ai_interpret_lenormand(card["name"], position, lang)
    await message.answer(f"🎴 <b>{card['name']}</b>\n{text}", parse_mode="HTML")
...
async def card_of_day_handler(message: Message):
    ...
    await message.answer(UI_TEXT[lang]["card_of_day"])
    await send_one_card(message, card, rev, UI_TEXT[lang]["card_of_day"], lang)


@dp.message(Command("lenormand"))
async def lenormand_card_handler(message: Message):
    lang = get_lang(message.from_user.id)
    card = draw_lenormand(1)[0]
    await send_one_lenormand_card(message, card, SPREAD_TITLES[lang]["lenormand"], lang)
...
async def send_named_spread(message: Message, title: str, positions: list[str], lang: str):
    ...
    for pos, (card, rev) in zip(positions, cards):
        await message.answer(f"<i>{pos}</i>", parse_mode="HTML")
        await send_one_card(message, card, rev, pos, lang)
...
async def cb_card(callback: CallbackQuery):
    ...
    card, rev = draw_cards(1)[0]
    await callback.message.answer(UI_TEXT[lang]["card_of_day"])
    await send_one_card(callback.message, card, rev, UI_TEXT[lang]["card_of_day"], lang)
...
async def cb_lenormand(callback: CallbackQuery):
    lang = get_lang(callback.from_user.id)
    card = draw_lenormand(1)[0]
    await send_one_lenormand_card(callback.message, card, SPREAD_TITLES[lang]["lenormand"], lang)
    await callback.answer()
...
async def cb_spread3(callback: CallbackQuery):
    lang = get_lang(callback.from_user.id)
    await send_named_spread(callback.message, SPREAD_TITLES[lang]["spread3"], SPREAD3_POSITIONS[lang], lang)
    await callback.answer()
...
async def cb_love(callback: CallbackQuery):
    lang = get_lang(callback.from_user.id)
    await send_named_spread(callback.message, SPREAD_TITLES[lang]["love"], LOVE_POSITIONS[lang], lang)
    await callback.answer()
