...
# Посилання на Mini App (сторінка вибору карт), яку хостить безкоштовний GitHub Pages.
# ВАЖЛИВО: заміни на своє реальне посилання після ввімкнення GitHub Pages —
# формат завжди https://<твій-github-юзернейм>.github.io/<назва-репозиторію>/
MINIAPP_URL = os.getenv("MINIAPP_URL", "https://ВСТАВ_СЮДИ_СВІЙ_GITHUB_PAGES_URL/")
...
def build_main_menu(lang: str) -> InlineKeyboardMarkup:
    ...
    b = MENU_BUTTONS[lang]
    rows = [
        [InlineKeyboardButton(text=b["card"], callback_data="menu_card", style="success")],
        [InlineKeyboardButton(text=b["lenormand"], callback_data="menu_lenormand", style="success")],
        [InlineKeyboardButton(text=b["spread3"], callback_data="menu_spread3", style="success")],
        [InlineKeyboardButton(text=b["love"], callback_data="menu_love", style="success")],
        [InlineKeyboardButton(text=b["decision"], callback_data="menu_decision", style="primary")],
        [InlineKeyboardButton(text=b["celtic"], callback_data="menu_celtic", style="primary")],
...
def build_webapp_button(needed: int, grid_size: int, lang: str, title: str | None) -> ReplyKeyboardMarkup:
    """
    Кнопка, що відкриває Mini App з анімованим вибором карт.
    ВАЖЛИВО: це має бути кнопка саме звичайної клавіатури (ReplyKeyboardMarkup),
    а не inline-кнопка під повідомленням — тільки такий тип кнопки дозволяє
    Mini App передати обрані карти назад у бота через tg.sendData().
    """
    from urllib.parse import quote
    url = f"{MINIAPP_URL}?n={needed}&grid={grid_size}&lang={lang}"
    if title:
        url += f"&title={quote(title)}"
    label = {
        "uk": "🎴 Обрати карти" if needed > 1 else "🎴 Обрати карту",
        "en": "🎴 Choose cards" if needed > 1 else "🎴 Choose a card",
        "es": "🎴 Elegir cartas" if needed > 1 else "🎴 Elegir una carta",
    }[lang]
    return ReplyKeyboardMarkup(
        keyboard=[[KeyboardButton(text=label, web_app=WebAppInfo(url=url), style="primary")]],
...
async def start_selection(target: Message, user_id: int, kind: str,
    ...
    """
    Запускає «гру вибору карт»: замість миттєвого результату відкриває
    Mini App з анімацією перевертання карт. Карти вже витягнуті й перемішані
    заздалегідь — порядок натискань у Mini App визначає, яка позиція яку
    carту отримає (не важливо, яку саме карту в сітці людина торкнеться —
    важливий лише порядок натискань).
    """
    ...
    header = f"🔮 <b>{title}</b>\n\n" if title else ""
    intro = {
        "uk": "Тисни кнопку нижче, щоб обрати карти:",
        "en": "Tap the button below to choose your cards:",
        "es": "Toca el botón de abajo para elegir tus cartas:",
    }[lang]
    await target.answer(
        f"{header}{intro}",
        parse_mode="HTML",
        reply_markup=build_webapp_button(needed, grid_size, lang, title),
    )


@dp.message(F.web_app_data)
async def web_app_data_handler(message: Message):
    """Приймає результат вибору карт із Mini App і показує розкриті карти."""
    import json
    ...
    try:
        ...
        logging.warning("Не вдалося розпарсити дані з Mini App")
        return
    ...
    for idx in picks:
        ...
        if len(positions) > 1:
            await message.answer(f"<i>{position}</i>", parse_mode="HTML")

        if session["kind"] == "tarot":
            await send_one_card(message, card, rev, position, lang)
        else:
            await send_one_lenormand_card(message, card, position, lang)
